...
namespace ManagemenHotelApp.AllUserControll
{
    public partial class UserRegisterRoom : UserControl
    {
        ...
        private void UserRegisterRoom_Load(object sender, EventArgs e)
        {
            ...
            String getCus = "Select idkhachhang, hotenkh, gioitinh, FORMAT (ngaysinh, 'dd/MM/yyyy') as ngaysinh, socccd, sodienthoai, quoctich, ghichu From KHACHHANG";
        ...
        {
            query = "select idkhachhang, hotenkh, gioitinh, FORMAT (ngaysinh, 'dd/MM/yyyy') as ngaysinh, socccd, sodienthoai, quoctich, ghichu from khachhang where hotenkh like N'" + txtNameSearch.Text + "%'";
            FillData(tbCustumer,query);
        }
        ...
        private void tbCustumer_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    String check = @"Select idhoadon,ngaythanhtoan from Khachhang, hoadonphong where khachhang.idkhachhang = hoadonphong.idkhachhang
        ...
        private void tbRoomNull_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            try
            {
                txtIDRoom.Text = tbRoomNull.Rows[e.RowIndex].Cells[1].Value.ToString();
                txtNameRoom.Text = tbRoomNull.Rows[e.RowIndex].Cells[2].Value.ToString();
                txtRoomType.Text = tbRoomNull.Rows[e.RowIndex].Cells[4].Value.ToString();
                txtPrice.Text = tbRoomNull.Rows[e.RowIndex].Cells[5].Value.ToString();
                txtSale.Text = tbRoomNull.Rows[e.RowIndex].Cells[6].Value.ToString();
            }
            catch (Exception ex)
            {
