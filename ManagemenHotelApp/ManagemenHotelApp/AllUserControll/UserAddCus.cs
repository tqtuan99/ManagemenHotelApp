...
namespace ManagemenHotelApp.AllUserControll
{
    public partial class UserAddCus : UserControl
    {
        ...
        private void UserAddCus_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                query = "Select idkhachhang, hotenkh, gioitinh, FORMAT (ngaysinh, 'dd/MM/yyyy') as ngaysinh ,socccd, sodienthoai, quoctich from Khachhang";
                DataSet ds = cn.getData(query);
                tbCustumer.DataSource = ds.Tables[0];
            }catch(Exception ex)
            {
                MessageBox.Show(ex +"Không kết nối được với database", "Waning!!!", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
